...
def get_args():
    ...
    parser.add_argument('--course',                      help='path to course directory that contains the course.yaml')
    parser.add_argument('--ids',    action='store_true', help='show ids of skills')
    parser.add_argument('--export', action='store_true', help='Check if we can export the course to JSON')
    parser.add_argument('--images',                      help='path to directory of images')
    parser.add_argument('--html',                        help='path to directory where to generate html report')
    ...
    for word in skill.words:
        for picture in word.pictures:
            if picture not in images["regular"]:
                warnings.append(f"Image {picture} used in {module.title}/{skill.name} does not exist")
    ...
    for module in course.modules:
        for skill in module.skills:
            ...
            if skill.id in skill_ids:
                errors.append(f"Duplicate id: {skill.id} in {module.title}/{skill.name} and in {skill_ids[skill.id]['module'].title}/{skill_ids[skill.id]['skill'].name}")
            if skill.name in skill_names:
                errors.append(f"Duplicate name: {skill.name} in {module.title}/{skill.name} and in {skill_names[skill.name]['module'].title}/{skill_names[skill.name]['skill'].name}")
            skill_ids[skill.id] = {
                'module': module,
                'skill': skill,
            }

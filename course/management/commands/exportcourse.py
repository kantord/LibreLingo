...
def get_course_data(course):
    ...
    return {
        ...
        "modules": [{
            ...
            "skills": [{
                **(get_imageset(skill)),
                "summary": [word.formInTargetLanguage for word in skill.learnword_set.all()] + [sentence.formInTargetLanguage for sentence in skill.learnsentence_set.all()],
...
def get_skill_data(skill, language_id, course):
    ...
    for learnsentence in skill.learnsentence_set.all():
        ...
        if len([clean_word(w)
                for w in learnsentence.meaningInSourceLanguage.split()]) >= 2:
            data = data + [{"type": "chips",
